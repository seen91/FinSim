...
| Counted units in the rate grammar (2026-07-14, amends "Curve hold") | **Any unit in a rate takes an integer count, on both sides of the parens.** "10 %/2m" is 10 % per two months; the quote compounds to the effective annual figure the engine keeps ((1+r)^(periods per year) − 1), and the compounding-in-parens means count × the unit's months — "(6m)", "(2m)", "(2yr)". Left off, the compounding still follows the quote unit, so "10 %/2m" steps every two months. **Sub-monthly compoundings collapse to the monthly tick** (the tick is untouched, as ever): "10 %/2m(2w)" credits monthly, arithmetically identical to the two-week credit over any horizon. **The rate's canonical tokens are `w`, `m`, `q`, `yr`** — a unit combines with a specific month via a dash ("(q-jan)" anchors quarters to Jan/Apr/Jul/Oct; a bare month "(apr)" is yearly anchored to April), and the field rewrites to these spellings; longhand input ("mo", "qtr", "week"…) stays accepted, and amount cadences keep their own "/mo"-style suffixes. The same counted grammar reaches the plain rate fields (asset growth, fee, interest) for their quote unit. The Grows field wears a hover ⓘ that states the grammar — pattern, examples, units — in financial terms ("credited", "compounded"). |
| Built-ins edit in place (2026-07-14, refines "One card — instances") | **A built-in's back is as editable as a design's — saving mints.** The Workshop no longer shows built-in math behind glass with a separate "copy to shelf" button: one editor serves both species, edits buffer as a draft, and the green save on a built-in mints a design from the edits *and re-points every table instance of the built-in at it* — "make it YOUR salary" is edit → save, one gesture, and "change once, all copies change" still holds by construction. The pristine template never mutates and stays dealable from the pile. An explicit **"copy"** mints from the template *without* re-pointing — a variant beside the default, for keeping the shipped card in play as-is. |
| The automatic debt line (2026-07-15, amends "one-line chart") | **Cash below zero draws itself on the chart.** While the plan is solvent the chart stays the one net-worth line; in any stretch where cash runs negative, the cash curve appears automatically as a red dotted line under a dotted zero baseline — a light wash down to zero, a label naming the depth and the months — and disappears the month the debt is repaid. No toggle, no legend, no permanent second line: net worth alone cannot distinguish a plan that borrows from one that doesn't (buying a fund moves money, it doesn't lose it), so the chart shows cash exactly where the difference bites. The y-axis grows a basement below zero only while debt exists, never shallower than 5 % of the chart so a small overdraft stays legible. The scrub readout appends ONE combined debt figure ("· debt −13,65 M") in any month the plan owes money — the debt cards' balances plus any cash overdraft — so hovering tells the whole borrowing story without splitting it into jargon. Cash flow and net worth remain separate ideas everywhere else: the cash dock stays the number, the chart stays net worth. |
| Margin card (2026-07-16, new kind; the table dresses it as a *companion*) | **Leverage is a card: a `margin` card pegs a loan to the assets below it in its hand.** Two dials — `ltv` (the share of the pegged assets' balance the loan is held at, 0 < ltv < 1) and `interest` (annual GrowthParam). It obeys the pipeline: played above its asset(s), it targets **asset cards below it in its hand** (nested hands included, rule-card scoping); at its position it draws the month's interest on the current loan balance from the running total (in full — the honest-overdraft convention). At month-end, after assets tick and take their deposits, the loan **rebalances to `ltv` × the pegged balance**: the borrowed delta is deposited straight into the pegged assets (pro-rata by balance; priced assets buy units) — broker credit, never passing through the running total — and a negative delta sells down to pay off, so a crash deleverages mechanically, under Monte Carlo too. Entering play, the same rule performs the initial borrow (closed form: loan = ltv⁄(1−ltv) × equity, since "loan = ltv × post-deposit balance" is circular). Reported like a debt — negative balance, counted in net worth and the chart's debt readout — and balance rules (ISK schablonskatt) keep taxing the **gross** asset balance: the tax-on-the-inflated-amount effect margin really has, at the rule's honest rate. Debt cards stay untouched — kind alone determines behavior, so the circular dependency gets its own kind, not a peg field on debt. Golden test: 5 % LTV at 1 % interest over a 7 % ± 15 % fund must reproduce the folded closed form on equity — (g − λr)⁄(1−λ) = 7,32 % expected, σ⁄(1−λ) = 15,8 % — deterministically. Engine name stays mechanical; the Workshop/table present it with companion flavor (a card leaning on its asset). |
| Packs retired as a file format (2026-07-17, supersedes "Pack format versioning") | **The table file is the one unit of sharing.** The separate pack file (`finsim-pack`, `packs.ts`) and the Workshop's export/import buttons are removed: the table file already carries the designs its instances play and the series those cards wear, so sharing cards is sharing a table — one export/import gesture, in the header. "Pack" survives as the design-level word for data bundles (locale rules, historical eras, scenarios) the game layers will ship; if a registry or standalone card-sharing need returns, it starts from the table-file envelope rather than reviving a second format. |
| The name IS the id (2026-07-17) | **A design's identity is its card name; a saved hand's is its name.** Same name = same card: importing a table file overrides the reader's same-named designs and saved hands instead of stacking uid-suffixed duplicates (the old `salary-a6a5c0ea` ids never matched across machines or resets, so every import doubled the shelf). Mechanics: the Workshop saves a design under its name and a rename is an identity change — every instance on the table and in the saved hands re-points in the same gesture; Workshop mints never clobber silently (a taken name suffixes to "name 2" — only an import overrides); older stores and files lift on read (`identity.ts`), no table-file version bump since ids stay opaque strings to older readers. Built-ins keep their prefixed refs (`pile:`/`preset:`) — a design named "Salary" and the built-in never collide. |
| A price is any curve (2026-07-17, widens "Backtesting"'s priced assets) | **`AssetCard.price` accepts the full Curve grammar, not only sampled data** — a car depreciating along `k·t + m` is a linear price, a scrap-value floor is an expression (`max(10000, 240000 - 1500*t)`), a revaluation schedule is steps; a bare `SampledRef` stays the historical-data shorthand every existing file uses (`priceCurveOf` folds it into the sampled curve — no file version bump). Semantics split by shape: a **sampled** price keeps the whole backtesting contract (history exact, `growth` as the post-data fallback, dice only on the extrapolated stretch); an **analytic** price is exact at every month in every future — it never ends, so `growth` idles and Monte Carlo never touches it (the deterministic-priced-assets rule extended, not amended). Evaluated at local `t` like every non-sampled curve. Companion fix, same day: **priced card faces say their numbers out loud** — start value in the headline, and stats for the data's coverage, first → last price, realized trend (%/yr CAGR), the after-data fallback, drift/rate for analytic shapes, and units held; the Workshop's asset editor grows a worth-mode switch (growth-rate balance ↔ units × price curve) with money-flavored curve fields (a price has no cadence), and in-play dials scale the analytic price's own parameters. |
| Data before its time idles, not errors (2026-07-18, revises "Backtesting") | **A sampled series that begins after the table start is no longer an error — the card idles until its first real month.** A priced card whose data starts later is treated as entering play when its data does: in play but worth nothing and taking nothing (deposits fall through to cash, honestly visible), then its pinned `initialUnits` materialize at the first real price and it plays inside its data as ever. A sampled *flow* is now silent on **both** sides of its data — 0 before it begins, 0 after it ends. Rationale: with analytic prices in the grammar, "history that hasn't happened yet" has one sensible meaning (a value pinned to future dates — a planned purchase, a scheduled valuation), and vetoing the whole table for it made the date picker fragile. The end-of-data contract is untouched: growth fallback from the last real price, dice only on that stretch; a *missing* world series is still a readable error — silence is only for data that exists but hasn't started. |
| Starter defaults: one relatable household (2026-07-14) | **The starter table is one median-ish household, no longer the five-fund acceptance fixture.** Salary +40 000/mo gross with a 3 % raise landing each January, income tax −30 %, living expenses −20 500/mo, a savings buffer (2,5 %/yr, filled 2 000/mo) — then the "Index fund investing" hand (100 % take) with the ISK rule card above ONE global index fund (7 % ± 15 %/yr) taking 80 %, so a trickle still lands as cash. The default goal drops to **2 MSEK** so the first render answers a reachable question ("the car costs 2 yr 3 mo"); preset members carry descriptions — the assumptions footnote — like blueprints do. The hand-checked five-fund golden scenario ("1 yr 3 mo" to 10 MSEK) lives on verbatim in the engine acceptance test and as a raw-card bridge test through the app's `runSim` — the M1 acceptance is unchanged; it just no longer doubles as the starter. |
...
| **Margin** | a loan pegged to the asset cards **below it** in its hand: held at `ltv` × their balance, rebalanced monthly (borrowed delta deposited into the assets, sell-down when they fall), interest drawn from the running total at its position; reported as a negative balance | Montrose portfolio margin at 5 % LTV — dressed on the table as a *companion* card |
