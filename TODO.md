...
- [ ] **One card — instances, not clones** (decided 2026-07-14; record in DESIGN.md §0 when it lands): kill the design/one-off split permanently. Today a played card is a deep clone of its template that may or may not wear a `design` stamp, so two identical-looking drains behave differently in the Workshop (found 2026-07-14: stamped copies follow design edits, unstamped one-offs edit alone). Target model:
