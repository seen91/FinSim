...
| The automatic debt line (2026-07-15, amends "one-line chart") | **Cash below zero draws itself on the chart.** While the plan is solvent the chart stays the one net-worth line; in any stretch where cash runs negative, the cash curve appears automatically as a red dotted line under a dotted zero baseline — a light wash down to zero, a label naming the depth and the months — and disappears the month the debt is repaid. No toggle, no legend, no permanent second line: net worth alone cannot distinguish a plan that borrows from one that doesn't (buying a fund moves money, it doesn't lose it), so the chart shows cash exactly where the difference bites. The y-axis grows a basement below zero only while debt exists, never shallower than 5 % of the chart so a small overdraft stays legible. The scrub readout appends the cash figure in those months. Cash flow and net worth remain separate ideas everywhere else: the cash dock stays the number, the chart stays net worth. |
