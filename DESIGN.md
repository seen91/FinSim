...
**Later:** stress tests beyond Monte Carlo (sequence-of-returns, historical replay of *your* plan through 2008), bots for solo, pack registry/sharing, more eras (1929, 1970s stagflation, 2008, Japan 1989, Argentina).
...
5. **Monte Carlo inputs (now M3-blocking, shapes the data model):** where do per-fund volatility estimates come from, and are the five funds' returns modeled as correlated (they should be — independent draws across index funds that track overlapping markets would badly understate risk)?
