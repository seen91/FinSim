...
| Backtesting: historical cards + replay-from-date (2026-07-12, pulls "historical replay of your plan" forward from Later into M3c) | **A backtest is the same table read against a real past instead of an assumed future.** Users create real cards by importing historical monthly data in the Workshop (a named series into `world.series`, minted as a priced-asset design), play them with fixed and/or percent takes like any card, then pick a date. The semantics are **re-anchoring, not time travel**: one global doc-level anchor (`doc.replayFrom`, absolute month, absent = off) shifts *every* historical series together so the anchor month lines up with the plan's start — "the next N years play out like 1999 onward did" — while the table, goal timeline, ghosts and verdicts stay on the present timeline. All series shift as one because cross-asset co-movement through a crash is the point. Applied at the sim boundary like tuning dials: the authored data never moves; `simulate` receives shifted copies via a pure engine helper (`reanchor`), with an up-front coverage check (`replayCoverage`) so no offerable anchor can run the sim off the end of a series. The Monte Carlo fan continues to come from growth-rate cards only — a replay is one real past, not a fan of futures; a table of only historical cards draws a single deterministic line. Import ships with the two data-trap warnings: use *total return* series, and currency is whatever the series is denominated in (no FX in v1). The *all-start-dates fan of pasts* (sequence-of-returns distribution) stays in Later as the natural sequel. |
...
**Later:** stress tests beyond Monte Carlo (the *all-start-dates fan of pasts* — a sequence-of-returns distribution reusing `MonteCarloRun` + the futures report; single-date historical replay shipped with M3c), bots for solo, pack registry/sharing, more eras (1929, 1970s stagflation, 2008, Japan 1989, Argentina).
