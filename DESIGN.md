...
| Workshop: each card is unique (2026-07-12) | **The design is the one true card.** The Workshop shelf shows each card once: your designs, then any one-off table cards (pile blueprints, presets) that have no design — played copies never appear and wear no 'in play' mark (a two-shelf split and an 'in play' plaque were both tried and rejected). A dealt card carries a `design` stamp back to its original (an app-level JSON key the engine never reads, like `glyph` and `tune`), and **editing a design lands on every copy in play** — each copy keeps its own id, its what-if dials and its set-aside state. Burning a design orphans its copies into one-offs, which become their own originals; duplicating a design mints an independent one. |
